...
from PySide6.QtCore import Qt, QUrl, Signal, QTimer, QPropertyAnimation, QEasingCurve, Property
from PySide6.QtWidgets import (
    QApplication, QMainWindow, QWidget, QVBoxLayout, QHBoxLayout,
    QLabel, QPushButton, QSlider, QScrollArea, QGroupBox, QFrame
)
...
class SoundPlayer(QWidget):
    ...
    def __init__(self, file_path: str, loop_mode: bool = False, is_folder: bool = False, parent=None):
    ...
    def play(self):
        ...
        if self.is_folder:
            ...
            if random_sound:
                ...
                print(f"Playing random sound from folder '{self.filename}': {Path(random_sound).stem}")
            else:
                print(f"No sounds found in folder '{self.filename}'")
                return
        
        self.media_player.play()
        if self.loop_mode:  # Only fade for ambient/music (folders are always effects, so no fade)
            try:
    ...
    def _pause(self):
        ...
        if self.loop_mode:  # Only fade for ambient/music
            try:
                ...
                pass
            self.fade_animation.finished.connect(lambda: self.media_player.pause())
            self.fade_animation.setStartValue(self._current_fade_volume)
            self.fade_animation.setEndValue(0.0)
            self.fade_animation.start()
        else:  # Effects (including folders) pause immediately
    ...
    def _update_volume(self):
       ...
       final_volume = slider_volume * self.master_volume * self._current_fade_volume
    ...
    def _on_media_status_changed(self, status):
        ...
        if (status == QMediaPlayer.EndOfMedia and 
            self.loop_mode and 
            self.media_player.playbackState() != QMediaPlayer.StoppedState):
            # Restart the media for looping
            self.media_player.setPosition(0)
            self.media_player.play()
        elif (status == QMediaPlayer.EndOfMedia and 
              self.is_folder and 
              not self.loop_mode):
...
class SoundSection(QWidget):
    ...
    def __init__(self, title: str, folder_path: str, loop_mode: bool = False, parent=None):
        super().__init__(parent)
        self.title = title
    ...
    def _load_sounds(self):
        ...
        if not os.path.exists(self.folder_path):
            ...
            no_folder_label = QLabel(f"📁 Folder '{self.folder_path}' not found")
        ...
        for i, item_path in enumerate(all_items):
            ...
            player = SoundPlayer(item_path, self.loop_mode, is_folder=is_folder)
...
class MusicBoardMainWindow(QMainWindow):
    ...
    def _setup_ui(self):
        ...
        effects_section = SoundSection("⚡ EFFECTS", "effects", loop_mode=False)
...
def create_sample_folders():
    """Create sample folders if they don't exist (for testing)."""
        ...
        if not os.path.exists(readme_path):
            with open(readme_path, "w") as f:
                ...
                if folder in ["ambient", "music"]:
                    f.write("✨ Files in this folder will loop automatically with 3-second fade in/out transitions.\n")
                else:
                    f.write("⚡ Files in this folder will play once at full volume immediately.\n")
                    f.write("📁 You can also add folders containing multiple sounds for random playback!\n")
                    f.write("🔄 Each time you click play on a folder, a random sound from that folder will play.\n")
                f.write("\n🎛️ Use individual volume sliders and master volume for perfect mixing.\n")
                f.write("🔤 Files and folders are automatically sorted in alphabetical order.\n")
